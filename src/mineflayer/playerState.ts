...
export class PlayerStateControllerMain {
  ...
  private botCreated () {
    ...
    // eye-height lowering, third-person camera pivot, and body-yaw lock (see
    // playerStateReactive.isMounted/mountedVehicleId usages in minecraft-renderer).
    // Without this, riding never visually registers even though mineflayer correctly
    // tracks bot.vehicle and disables local ground physics.
    bot.on('mount', () => {
      this.reactive.isMounted = true
      this.reactive.mountedVehicleId = (bot as any).vehicle?.id
    })
    bot.on('dismount', () => {
      this.reactive.isMounted = false
      this.reactive.mountedVehicleId = undefined
    })
